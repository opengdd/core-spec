...
const STRATEGIES = new Set(["reflow", "stack", "summary"]);
...
export function validatePanelDescriptor(panel, ids = new Set()) {
  ...
  if (!plainObject(panel.compact) || !STRATEGIES.has(panel.compact.strategy) || typeof panel.compact.note !== "string" || !panel.compact.note.trim()) fail(panel, "compact");
...
export function createPanelHost({ document, view, panels, inspector, sidebar, services, packageService, hostInfo, internal = {}, report }) {
  ...
  const switcher = inspector.querySelector("[data-panel-switcher]");
  ...
  function scopedServices(panel, instance, { creator = false } = {}) {
    ...
    result.selection = result.selection && {
      ...result.selection,
      select(next, options) {
        return services.selection.select(next, { ...options, origin: { surface: "panel", panel: { id: panel.descriptor.id, surface: instance.surface } } });
      }
    };
    ...
    if (result.validation) {
      result.validation.contribute = findings => services.validation.contribute(findings, panel.descriptor.id);
    }
  ...
  function mount(panel, surface, parent, badge, { actionOnly = false } = {}) {
    ...
    const instance = { surface, controller, element, state, badge, cancels: new Set(), failed: false, handle: undefined, empty: false };
    ...
    try {
      ...
      if (instance.handle && typeof instance.handle.then === "function") {
        instance.handle.then(handle => { instance.handle = handle; }).catch(error => failInstance(panel, instance, error));
      }
    } catch (error) { failInstance(panel, instance, error); }
    return instance;
  }
  ...
  function renderSidebarPanel(panel) {
    ...
    group.className = "opengdd-author-outline-group opengdd-author-panel-group";
  ...
  function renderInspector() {
    ...
    if (box.width > 0 && box.height > 0) fit = box.width >= 16 * rootSize && box.height >= 12 * rootSize;
    ...
    switcher.replaceChildren(...matches.map(panel => {
      ...
      option.textContent = panel.descriptor.title;
      option.selected = panel.descriptor.id === chosenId;
      return option;
    }));
    switcher.hidden = matches.length < 2;
    const chosen = matches.find(panel => panel.descriptor.id === chosenId);
  ...
  async function registerAll(source) {
    ...
    catch (error) { report(`Panels could not be loaded: ${error.message}`, true); return; }
    if (destroyed) return;
    if (values === undefined) values = [];
    if (!Array.isArray(values)) { report("host.panels must provide an array of panel descriptors.", true); return; }
