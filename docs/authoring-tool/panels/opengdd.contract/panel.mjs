...
import {
  contractQuestionLiveness, contractTestInputField, firstSentence, liveContractTemplates, pointerFor,
...
function prospectiveValueIssue(adoption, name, value) {
  ...
  for (const line of Object.values(plainObject(adoption.rules) ? adoption.rules : {})) {
    ...
    if (!names.includes(name) || names.some(field => !values.has(field))) continue;
    try {
...
function blockingAnswer(adoption, questionId, asked) {
  ...
  const flags = questions[questionId]?.when?.flag;
  if (!plainObject(flags)) return undefined;
  for (const [dependency, allowed] of Object.entries(flags)) {
    if (Object.hasOwn(answers, dependency)
      && (!asked.has(dependency) || !allowed.includes(answers[dependency]))) {
      return { id: dependency, option: answers[dependency] };
    }
  }
  return undefined;
}
...
export default {
  ...
  compact: { strategy: "reflow", note: "The worksheet reflows into the inspector drawer." },
  ...
  create(context) {
    ...
    function questionSection(adoption, asked) {
      ...
      for (const [id, question] of Object.entries(plainObject(adoption.questions) ? adoption.questions : {})) {
        ...
        if (!isAsked) {
          ...
          card.prepend(badge);
          const blocked = blockingAnswer(adoption, id, asked);
          if (blocked) {
            const cause = adoption.questions[blocked.id];
            const reason = element(document, "p", undefined, "opengdd-author-contract-not-asked-reason");
            const meaning = firstSentence(cause?.options?.[blocked.option]?.meaning);
            const copy = CONTRACT_COPY.doesntApplyReason(meaning, cause?.asks ?? blocked.id);
            reason.append(copy.before);
            const link = element(document, "button", copy.question, "opengdd-author-contract-jump");
            link.type = "button";
            link.addEventListener("click", () => jump(questionNode(blocked.id)));
    ...
    function valuesSection(adoption) {
      ...
      section.append(host);
      const fields = Object.entries(declarations).filter(([name]) => !name.startsWith("_")).map(([name, declaration]) => ({
        key: name, label: name, type: "number", help: declaration.description,
        binding: { file: selectedFile, pointer: pointerFor("values", name) },
        clearRemoves: true, commitOnBlur: true,
        validate: value => prospectiveValueIssue(adoption, name, value),
        labels: { change: CONTRACT_COPY.undo.changeValue, clear: CONTRACT_COPY.undo.changeValue },
        write: async (value, detail) => {
          const issue = prospectiveValueIssue(adoption, name, value);
          try {
            return await write(adoption, ["values", name], value, {
              remove: detail.remove, label: CONTRACT_COPY.undo.changeValue,
              allowTodo: detail.remove, allowDependent: true
            });
    ...
    function render() {
      ...
      intro.append(element(document, "p", adoption.summary ?? ""), mode,
        element(document, "p", `${adoption.contract}, version ${adoption.version}`, "opengdd-author-contract-version"));
      body.append(intro);
      ...
      questionSection(adoption, asked);
      valuesSection(adoption);
