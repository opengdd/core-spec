...
function readField(packageService, field) {
  if (typeof field.read !== "function") return readBinding(packageService, field.binding);
  const state = field.read();
  if (state && typeof state === "object" && Object.hasOwn(state, "exists") && Object.hasOwn(state, "value")) return state;
  return { exists: state !== undefined, value: state };
}
  ...
  return Object.freeze({
    create(element, { fields, onChange, labels = {} } = {}) {
      ...
      for (const field of fields) {
        ...
        if (field.helpShared !== undefined && (typeof field.helpShared !== "string" || !field.helpShared)) {
          throw new Error(`Form field ${field.key} needs a non-empty shared help id.`);
        }
        if (field.rowLabel !== undefined && (typeof field.rowLabel !== "string" || !field.rowLabel)) {
          throw new Error(`Form field ${field.key} needs a non-empty row label.`);
        }
      ...
      const requiredMark = field => {
        ...
        name.className = "opengdd-author-visually-hidden";
        name.textContent = field.requiredLabel ?? INSPECTOR_COPY.required;
        required.append(mark, name);
        return required;
      };

      const setLabelText = (label, field) => {
        if (!field.required) {
          label.textContent = field.label;
          return;
        }
        const words = String(field.label).trim().split(/\s+/u);
        const last = words.pop() ?? "";
        if (words.length) label.append(`${words.join(" ")} `);
        const tail = document.createElement("span");
        tail.className = "opengdd-author-form-label-tail";
        tail.append(last, " ", requiredMark(field));
        label.append(tail);
      };
      ...
      function fieldChrome(parent, field, { cell = false, suffix = "", group = false } = {}) {
        ...
        box.className = cell ? "opengdd-author-form-cell" : "opengdd-author-form-field opengdd-author-form-row-grid";
        box.dataset.formField = field.key;
        box.dataset.formType = field.type;
        const id = `${formId}-${pointerSegment(field.key)}${suffix}`;
        let label;
        if (!cell && !field.helpShared) {
          label = document.createElement(group ? "p" : "label");
          if (group) label.id = `${id}-label`;
          else label.htmlFor = id;
          setLabelText(label, field);
          box.append(label);
        }
        const help = document.createElement("p");
        help.className = "opengdd-author-form-help";
        help.dataset.formHelp = "";
        help.id = field.helpShared ? `${formId}-${pointerSegment(field.helpShared)}-help` : `${id}-help`;
        help.textContent = helpText(field.help) ?? "";
        help.hidden = !help.textContent;
        if (cell) help.classList.add("opengdd-author-visually-hidden");
        ...
        const attach = control => {
          ...
          } else if (cell || field.helpShared || field.ariaLabel) control.setAttribute("aria-label", field.ariaLabel ?? field.label);
          control.setAttribute("aria-describedby", describedBy);
          if (field.monospace) control.classList.add("opengdd-author-form-monospace");
          if (field.disabled) control.disabled = true;
          box.append(control, ...(field.helpShared ? [] : [help]), note, error);
          if (field.action) {
      ...
      function refreshStatus(chrome, field, value) {
        if (typeof field.status !== "function") return;
        chrome.note.classList.add("opengdd-author-form-help");
        chrome.note.textContent = field.status(value) ?? "";
      ...
      function scalarRenderer(parent, field, options = {}) {
        ...
        const mounted = typeof field.mount === "function"
          ? field.mount({ document, box: chrome.box, control })
          : undefined;
        let shownValue;
        const fitTextarea = () => {
          if (control.tagName?.toLowerCase() !== "textarea" || !control.style) return;
        ...
        const refresh = () => {
          ...
          fitTextarea();
          mounted?.refresh?.(current.value);
          validateLater(field, current.value, chrome.setError, true);
          refreshStatus(chrome, field, current.value);
        };
        ...
        control.addEventListener("input", () => {
          fitTextarea();
          return field.commitOnBlur
            ? validateLater(field, control.value, chrome.setError)
            : change().catch(error => chrome.setError(error.message));
        });
        const row = chrome.box.parentNode?.dataset?.formRow !== undefined ? chrome.box.parentNode : undefined;
        const leaveRow = event => {
          if (row?.contains(event.relatedTarget)) return;
          burst += 1;
          if (control.value === format(shownValue)) return validateLater(field, shownValue, chrome.setError, true);
          return change(true).catch(error => chrome.setError(error.message));
        };
        if (field.commitOnBlur && field.commitOnRowExit && row) row.addEventListener("focusout", leaveRow);
        control.addEventListener("blur", event => {
          if (field.commitOnBlur && field.commitOnRowExit && row) return;
          burst += 1;
        ...
        refresh();
        queueMicrotask(fitTextarea);
        return { refresh, destroy() {
          if (row) row.removeEventListener("focusout", leaveRow);
          mounted?.destroy?.();
        } };
      }
      ...
      function enumRenderer(parent, field, options = {}) {
        ...
        const refresh = () => {
          current = readField(packageService, field).value;
          ...
          if (field.options.length > 6) {
            ...
            for (const value of field.options) {
              ...
              button.addEventListener("click", () => current === value && field.required
                ? undefined
                : commit(current === value ? null : value));
      ...
      function linkRenderer(parent, field, options = {}) {
        ...
        const dangling = value => typeof value === "string" && value && !knownIds.includes(value)
          ...
          : "";
        const refresh = () => {
          const state = readField(packageService, field);
      ...
      function referenceRenderer(parent, field, options = {}) {
        ...
        const mounted = typeof field.mount === "function"
          ? field.mount({ document, box: chrome.box, control: select })
          : undefined;
        ...
        const refresh = () => {
          ...
          mounted?.refresh?.(current);
          validateLater(field, state.value, chrome.setError, true);
          refreshStatus(chrome, field, state.value);
        ...
        refresh();
        return { refresh, destroy: () => mounted?.destroy?.() };
      }
      ...
      function linesRenderer(parent, field, options = {}) {
        ...
        const refresh = (force = false) => {
          ...
          const current = readField(packageService, field).value;
            ...
            for (const nested of field.options.fields) {
              ...
              if (nested.type === "lines") {
                ...
                const nestedField = {
                  ...
                  labels: nested.labels ?? field.labels,
      ...
      // Shared help renders once inside the first row of its family. The first
      // row owns the labels and hidden descriptions; later rows keep the same
      // control columns without adding empty heading blocks.
      const rowFamily = field => (field.row ?? "").replace(/-\d+$/u, "");
      const sharedFields = [...new Map(fields.filter(field => field.helpShared)
        .map(field => [field.helpShared, field])).values()];
      const renderedShared = new Set();
      const renderSharedHelp = family => {
        const pending = sharedFields.filter(field => !renderedShared.has(field.helpShared) && rowFamily(field) === family);
        if (!pending.length) return undefined;
        const shared = document.createElement("div");
        shared.className = "opengdd-author-form-shared-help";
        shared.dataset.formSharedHelp = "";
        for (const field of pending) {
          renderedShared.add(field.helpShared);
          const column = document.createElement("div");
          column.className = "opengdd-author-form-shared-column";
          const label = document.createElement("p");
          label.className = "opengdd-author-form-shared-label";
          setLabelText(label, field);
          const help = document.createElement("p");
          help.className = "opengdd-author-form-help";
          help.dataset.formHelp = "";
          help.id = `${formId}-${pointerSegment(field.helpShared)}-help`;
          help.textContent = helpText(field.help) ?? "";
          label.title = help.textContent;
          label.setAttribute("aria-describedby", help.id);
          column.append(label, help);
      ...
      for (const field of fields) {
        if (field.row === undefined) {
          row = undefined;
          rowKey = undefined;
          const shared = field.helpShared ? renderSharedHelp("") : undefined;
          if (shared) root.append(shared);
          renderField(root, field);
          continue;
        }
        if (!row || rowKey !== field.row) {
          const family = rowFamily(field);
          const familyShared = sharedFields.filter(candidate => rowFamily(candidate) === family);
          const shared = renderSharedHelp(family);
          row = document.createElement("div");
          row.className = "opengdd-author-form-row";
          if (familyShared.length) {
            row.classList.add("opengdd-author-form-row--shared");
            row.dataset.formSharedColumns = String(familyShared.length);
          }
          if (shared) {
            row.dataset.formSharedFirst = "";
            row.append(shared);
          }
          row.dataset.formRow = field.row;
          if (field.rowLabel) {
            row.setAttribute("role", "group");
            row.setAttribute("aria-label", field.rowLabel);
          }
          root.append(row);
          rowKey = field.row;
        }
