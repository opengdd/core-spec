import { contractAdoptionActivation, contractRowWhenSatisfied, pointerFor,
  writeContractDesignerValue } from "./contract-worksheet.mjs";
import { own, plainObject } from "./json-path.mjs";
...
function settleInapplicable(row, schema, answers, activation) {
  for (let pass = 0; pass < Object.keys(schema).length; pass += 1) {
    ...
    for (const [key, shape] of Object.entries(schema)) {
      if (!plainObject(shape?.when) || !own(row, key)
        || contractRowWhenSatisfied(shape.when, answers, activation.asked, row, activation.facts)) continue;
      delete row[key];
      removed = true;
    }
    if (!removed) break;
...
export function createContractRows({ file, set, schema, readAdoption, internal, copy }) {
  ...
  const writeBoundary = (adoption, options) => writeContractDesignerValue({
    internal, adoption, file, allowRowTodo: true, allowAskedAnswerTodo: true, ...options
  });
  ...
  const adapter = {
    ...
    async add(rows, label = copy.undo.addRow) {
      ...
      const values = normalizeRows(rows, schema, existing, copy)
        .map(row => settleInapplicable(row, schema, answers, activation));
    ...
    field(id, key, shape) {
      ...
      const answers = plainObject(adoption.answers) ? adoption.answers : {};
      const activation = contractAdoptionActivation(adoption);
      if (plainObject(shape.when) && !contractRowWhenSatisfied(shape.when, answers, activation.asked, row, activation.facts)) {
