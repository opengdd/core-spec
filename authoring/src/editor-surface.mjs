...
import {
  clampedPositionToOffset, lineBounds, lineStarts, lineText, offsetToPosition, rangeFromOffsets, updateLineStarts
...
// Existing names an unknown citation may have meant: the same opening,
// the same text inside, or one or two characters off. Exported for the checks.
export function nearbyNames(name, table, limit = 5) {
  ...
  const lowercased = table.map(entry => String(entry.name).toLowerCase());
  const ranked = prefixContainsRanks(wanted, lowercased, { reverseContains: true });
  for (const [index, entry] of table.entries()) {
    const candidate = lowercased[index];
    if (candidate === wanted) continue;
    let rank = ranked[index];
    if (rank === undefined) {
      const off = distance(wanted, candidate);
      if (off > 2) continue;
      rank = 1 + off;
    }
    ranked[index] = { entry, rank };
  }
  return ranked.filter(item => typeof item === "object")
    .sort((left, right) => left.rank - right.rank || left.entry.name.length - right.entry.name.length
    || left.entry.name.localeCompare(right.entry.name))
    .slice(0, limit).map(item => item.entry);
}

// The editor completion list and nearby-name repair share the same two cheap
// ranks over the caller's cached lowercase table. Only nearbyNames adds edit
// distance for entries these ranks do not match.
// Typed completions match inside a name from the first character; a quick-fix
// guess (`reverseContains`) needs three, since a one- or two-character
// citation is inside too many names to mean any of them.
function prefixContainsRanks(wanted, lowercasedTable, { reverseContains = false } = {}) {
  const contains = !reverseContains || wanted.length >= 3;
  return lowercasedTable.map(candidate => candidate.startsWith(wanted) ? 0
    : contains && (candidate.includes(wanted) || (reverseContains && candidate.length >= 3 && wanted.includes(candidate))) ? 1
      : undefined);
}

// Typed completions: names that open with the prefix first, then names that
// contain it, from the first character typed. Exported for the checks.
export function completionMatches(prefix, table, lowercasedTable) {
  const ranks = prefixContainsRanks(prefix.toLowerCase(), lowercasedTable);
  return table.filter((_entry, index) => ranks[index] === 0)
    .concat(table.filter((_entry, index) => ranks[index] === 1));
}

function completionEntryLabel(entry) {
  const definition = entry.definitions[0] ?? {};
  return { definition, label: CREATION_COPY.kindLabels[definition.kind] ?? CREATION_COPY.kindLabels.name };
}
...
export function createEditorSurface({
  ...
  function positionPopup(element) {
    ...
    // The popup is fixed to the viewport so no panel, band or sheet can cover
    // it. Below the caret line when that fits on screen, above it otherwise.
    const box = ui.textarea.getBoundingClientRect();
    const viewportHeight = view.innerHeight;
    const caretTop = Math.max(0, Math.min(top, ui.textarea.clientHeight));
    const fitsBelow = box.top + caretTop + element.offsetHeight <= viewportHeight;
    const above = caretTop - lineHeight - element.offsetHeight;
    const placed = fitsBelow || box.top + above < 0 ? caretTop : above;
    element.style.top = `${Math.round(box.top + placed)}px`;
    element.style.left = `${Math.round(box.left + left)}px`;
  }
  ...
  function offerCompletions() {
    ...
    const matches = completionMatches(context.prefix, viewSupplier()?.completionNames ?? [], lowercaseNames());
    ...
    ui.completions.innerHTML = countLine + visible.map(entry => {
      const { definition, label } = completionEntryLabel(entry);
  ...
  function renderDialog(focus = false) {
    ...
    const suggestions = quickfix.suggestions?.length
      ? `<p>${CREATION_COPY.useExisting}</p><div class="opengdd-author-quickfix-suggestions">${quickfix.suggestions.map(entry => {
        const { label } = completionEntryLabel(entry);
  ...
  function openDialog(anchor) {
    ...
    quickfix = {
      name: anchor.name,
      range: anchor.range,
      suggestions: nearbyNames(anchor.name, viewSupplier()?.completionNames ?? []),
  ...
  // Replaces the unknown citation the dialog is about with an existing name.
  // The anchor's range locates it; a dialog opened by name alone (a panel's
  // create request) falls back to the backticked spelling nearest the caret.
  async function useExistingName(name) {
    const pending = quickfix;
    if (!pending) return;
    const value = ui.textarea.value;
    const wanted = `\`${pending.name}\``;
    let start = -1;
    if (pending.range) {
      const at = clampedPositionToOffset(value, pending.range.start);
      if (value.slice(at, at + wanted.length) === wanted) start = at;
    }
    if (start < 0) {
      const caret = pending.selection.start;
      let best = -1;
      for (let at = value.indexOf(wanted); at >= 0; at = value.indexOf(wanted, at + 1)) {
        if (best < 0 || Math.abs(at - caret) < Math.abs(best - caret)) best = at;
      }
      start = best;
    }
    if (start < 0) { dismissDialog(); report(CREATION_COPY.citationGone(pending.name), true); return; }
    const next = `${value.slice(0, start)}\`${name}\`${value.slice(start + wanted.length)}`;
    const caret = start + name.length + 2;
    dismissDialog(false);
    ui.textarea.value = next;
    await queueEdit({ label: CREATION_COPY.useExistingAction(name), path: currentPath(), text: next });
    textRevision += 1;
    overlay.lineStarts = [];
    schedule({ changed: true });
    ui.textarea.focus({ preventScroll: true });
    ui.textarea.setSelectionRange(caret, caret);
    creationApplied({ changed: true, showCompletion: false });
  }
  ...
  on(ui.editor, "click", event => {
    ...
    const use = event.target.closest("[data-quickfix-use]");
    if (use && quickfix) { useExistingName(use.dataset.quickfixUse).catch(error => report(error.message, true)); return; }
  ...
  // Fixed popups follow the editor when the page or a host panel scrolls or resizes.
  const repositionPopups = () => { if (completionButtons().length) positionPopup(ui.completions); if (quickfix) positionPopup(ui.quickfix); };
  on(view, "resize", repositionPopups);
  on(document, "scroll", repositionPopups, { capture: true, passive: true });
