...
export function extractContractDefinition(text) {
  ...
  let preserved = !reordered ? receivedDefinitionText(text) : undefined;
  // Some adoptions put pack (or other definition fields) after the answers.
  // In that case the prefix alone is not the complete definition.
  if (preserved && JSON.stringify(parsed(preserved)) !== JSON.stringify(value)) preserved = undefined;
...
export function contractSourceName(path) {
  return /(?:^|[/\\])([a-z0-9]+(?:-[a-z0-9]+)*)\.json$/u.exec(String(path ?? ""))?.[1] ?? "";
}

function definitionIdentity(value) {
  const strip = item => Array.isArray(item) ? item.map(strip) : plainObject(item)
    ? Object.fromEntries(Object.entries(item).filter(([key]) => !key.startsWith("_")).map(([key, child]) => [key, strip(child)])) : item;
  return JSON.stringify(strip(Object.fromEntries(CONTRACT_DEFINITION_FIELDS
    .filter(key => Object.hasOwn(value, key)).map(key => [key, value[key]]))));
}

function copiedDesignerPart(value, sourceName, name) {
  let remapped = 0;
  const copy = item => {
    if (typeof item === "string" && sourceName && sourceName !== name
      && (item === `contracts.${sourceName}` || item.startsWith(`contracts.${sourceName}.`))
      && /^contracts\.[a-z0-9]+(?:-[a-z0-9]+)*(?:\.[a-z0-9]+(?:-[a-z0-9]+)*)*$/u.test(item)) {
      remapped += 1;
      return `contracts.${name}${item.slice(`contracts.${sourceName}`.length)}`;
    }
    if (Array.isArray(item)) return item.map(copy);
    if (plainObject(item)) return Object.fromEntries(Object.entries(item).map(([key, child]) => [key, key.startsWith("_") ? structuredClone(child) : copy(child)]));
    return item;
  };
  const designer = Object.fromEntries(Object.entries(value).filter(([key]) => DESIGNER_FIELDS.has(key))
    .map(([key, item]) => [key, key === "answers" ? structuredClone(item) : copy(item)]));
  for (const [key, item] of designerAnnotationEntries(value)) designer[key] = structuredClone(item);
  return { designer, remapped };
}

export function prepareContractAddition({ files, definitionText, packText, name, sourceFilename, startUnanswered = false }) {
  ...
  const source = classifyContractText(definitionText);
  const preservesAdoption = source.kind === "adoption" && !startUnanswered;
  ...
  if (preservesAdoption) for (const [file, text] of files) {
    if (!/^contracts\/[^/]+\.json$/u.test(file) || file.endsWith(".pack.json")) continue;
    const current = classifyContractText(text);
    if (current.kind === "adoption" && current.value.contract === incoming.contract && current.value.version === incoming.version
      && definitionIdentity(current.value) !== definitionIdentity(incoming)) {
      return Object.freeze({ ok: false, reason: CONTRACT_COPY.definitionConflict(file) });
    }
  }
  const definition = preservesAdoption ? incoming : existing?.value ?? incoming;
  let pack;
  ...
  const definitionTextToWrite = preservesAdoption ? received.text : existing?.text ?? received.text;
  const preserveReceivedDefinition = preservesAdoption || !(existing?.reordered ?? received.reordered);
  const copied = preservesAdoption ? copiedDesignerPart(source.value, contractSourceName(sourceFilename), normalizedName) : null;
  return Object.freeze({
    ok: true,
    name: normalizedName,
    path: `contracts/${normalizedName}.json`,
    adoptionText: copied ? appendDesignerValue(definitionTextToWrite, copied.designer, definition)
      : preserveReceivedDefinition ? appendDesignerPart(definitionTextToWrite, definition) : contractScaffoldText(definition),
    preservesAdoption,
    remappedReferences: copied?.remapped ?? 0,
    definition,
...
export function collectContractSources(files) {
  ...
  for (const [path, value] of files) {
    ...
    if (classification.kind === "pack") packs.push({ path, text: value, value: classification.value });
      ...
      definitions.push({ path, sourceFilename: path || undefined, text, value: classification.value, kind: classification.kind });
    }
  }
