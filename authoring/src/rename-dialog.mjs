...
import { confirmationHost, element, renderBackticks } from "./dom.mjs";

export function openRenameDialog({ document, anchor, references, name, initial = "", normalize, onApplied, onCancel }) {
  ...
  let plan;
  const current = normalize ? normalize(initial)
    : name.includes("#") ? name.split("#", 2)[1].split(".").at(-1) : name.split(".").at(-1);
  const update = () => {
    const field = name.includes("#");
    const next = normalize ? normalize(input.value) : field ? input.value.trim() : kebabName(input.value);
  ...
  const row = confirmationHost(anchor);
  let container = box;
  if (row?.tagName === "TR") {
