...
export function setJsonValue(text, pointer, value) {
  const node = resolve(walkJsonDocument(text), pointer);
...
export function insertJsonValue(text, pointer, keyOrIndex, value, options = {}) {
  const container = resolve(walkJsonDocument(text), pointer);
  ...
  if (options.source !== undefined) {
    if (typeof options.source !== "string") throw new Error("An inserted JSON source must be text.");
    let suppliedValue;
    try { suppliedValue = JSON.parse(options.source); }
    catch { throw new Error("An inserted JSON source must contain one JSON value."); }
    if (rendered(suppliedValue) !== rendered(value)) throw new Error("An inserted JSON source must represent the inserted value.");
    suppliedText = options.source;
  }
  const valueText = suppliedText ?? prettyText() ?? (options.recordSpacing
  ...
  if (container.type === "object") {
    ...
    if (options.before !== undefined) {
      const index = container.properties.findIndex(property => property.key === options.before);
      if (index < 0) throw new Error(`Object member ${JSON.stringify(options.before)} does not exist.`);
      const start = itemStart(container, index);
      return splice(text, start, start, `${snippet}${memberSeparator(text, container, Math.max(0, index - 1))}`);
    }
