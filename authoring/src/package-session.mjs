...
export function createPackageSession({
  ...
  const packageOptions = () => {
    ...
    return [
      ...builtins.map(item => {
        ...
        return { id: item.id, label: draft ? `${draft.title} — local draft${update}` : item.title };
      }),
  ...
  function install(value, { saved = false } = {}) {
    if (closed || isClosed()) return;
    flush();
    stopEditing();
    revision += 1;
    ...
    saveStatus = saved ? "Saved in this browser" : "Not saved";
    publish("package");
    return pkg;
  }
  ...
  return Object.freeze({
    ...
    async open(value, { force = false } = {}) {
      ...
      if (value === pkg.id && !force) return pkg;
      ...
      install(opened, { saved: Boolean(draft) });
      if (draftNeedsExampleUpdate(draft, builtin)) {
        report("This local draft predates the listed package. Export it before resetting to the original files if you want to keep its changes.");
      }
      return pkg;
    },
    ...
    async deleteDraft({ request = false, cancel = false, confirm = false, id = pkg.id } = {}) {
      ...
      if (selected) await this.open(selected, { force: true });
