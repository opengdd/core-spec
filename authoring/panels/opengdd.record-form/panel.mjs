...
import { anchoredConfirmation, element } from "../../src/dom.mjs";
import { entityHeader } from "../../src/inspector-kit.mjs";
...
import { referenceGroup } from "../../src/inspector-groups.mjs";
...
export default {
  ...
  create(context) {
    ...
    function recordHeader(collection, recordName, file) {
      const address = `collections.${collection}.${recordName}`;
      const header = entityHeader({ document, context, entity: {
        id: address, title: recordName, kindLabel: "record", file
      }, actions: [{
        label: COLLECTION_COPY.rename, dataset: "recordRename", run: anchor => openRenameDialog({
          document, anchor, references: context.services.references, name: address, initial: recordName,
          onApplied: next => selectFile("collection-record", next, `collections/${collection}/${next}.json`)
        })
      }, {
        label: COLLECTION_COPY.remove, dataset: "recordRemove", run: async anchor => {
          const sites = context.services.references.usages(address);
          const mentions = sites.filter(site => site.channel === "prose").length;
          const links = sites.filter(site => site.kind === "link-value").length;
          const accepted = await anchoredConfirmation({ document, anchor,
            question: COLLECTION_COPY.removeRecord(recordName, mentions, links), actions: [
              { label: COLLECTION_COPY.remove, value: true, danger: true },
              { label: COLLECTION_COPY.cancel, value: false }
            ], datasetKey: "recordConfirmation" });
          if (!accepted) return;
          const transaction = context.services.edits.begin(COLLECTION_COPY.undo.removeRecord);
          transaction.file(file).remove();
          await transaction.commit();
          context.services.selection.select(null);
        }
      }, {
        label: INSPECTOR_COPY.showInFile(file), dataset: "openInEditor",
        run: () => selectFile("file", recordName, file)
      }, {
        label: RECORD_FORM_COPY.duplicate, dataset: "recordDuplicate", run: anchor => context.internal.openCollectionRecord({
          anchor, collection, prefill: `${recordName}-copy`, sourceFile: file,
          undo: RECORD_FORM_COPY.undo.duplicateRecord
        })
      }] });
      header.classList.add("opengdd-author-record-breadcrumb");
      return header;
    }
    ...
    function renderNotChecked(record) {
      ...
      const rows = keys.map(key => {
        const item = element(document, "span", undefined, "opengdd-author-reference-detail");
        const name = element(document, "code", key);
        const value = element(document, "code", JSON.stringify(record[key]));
        item.append(name, " · ", value);
        return item;
      });
      const section = referenceGroup(context, { label: RECORD_FORM_COPY.notChecked,
        help: RECORD_FORM_COPY.notCheckedHelp, rows });
      section.classList.add("opengdd-author-record-not-checked");
      body.append(section);
    }
    ...
    function renderUnknownFields(files, collection, file, record, schema, labelFile) {
      ...
      const section = referenceGroup(context, { label: RECORD_FORM_COPY.notDescribed,
        help: RECORD_FORM_COPY.notDescribedHelp, rows });
      section.classList.add("opengdd-author-record-unknown");
      body.append(section);
    }
    ...
    function render() {
      ...
      body.append(recordHeader(collection, recordName, file));
