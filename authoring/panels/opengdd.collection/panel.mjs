...
export default {
  ...
  needs: ["selection", "edits", "references", "forms", "validation"],
  ...
  create(context) {
    ...
    async function confirmRemove(anchor, detail) {
      return anchoredConfirmation({ document, anchor, question: detail.question, actions: [
        { label: COLLECTION_COPY.remove, value: true, danger: true }, { label: COLLECTION_COPY.cancel, value: false }
    ...
    function fieldRows(tbody, collection, level, parentPath = [], depth = 0) {
      for (const [field, shape] of Object.entries(level)) {
        ...
        const name = element(document, "input", undefined, "opengdd-author-identifier"); name.value = field; name.size = Math.max(4, Math.min(16, field.length)); name.dataset.fieldName = ""; name.setAttribute("aria-label", `field name ${field}`);
        ...
        const actionCell = element(document, "td"); const remove = actionButton(COLLECTION_COPY.remove, async anchor => {
          ...
          if (mirrors.length) {
            const choice = await anchoredConfirmation({ document, anchor,
              ...
              { label: COLLECTION_COPY.removeBoth, value: "both", danger: true }, { label: COLLECTION_COPY.keepBoth, value: "keep" }
              ], datasetKey: "collectionConfirmation" });
    ...
    function renderFieldsSection(parent, collection, schema) {
      if (!plainObject(schema)) {
        const action = actionButton(COLLECTION_COPY.describeFields, anchor => describeDialog(anchor, collection), "describeFields");
        const section = referenceGroup(context, { label: COLLECTION_COPY.notDescribed,
          help: COLLECTION_COPY.notDescribedHelp, rows: [], action });
        section.dataset.collectionFields = "";
      ...
      section.append(actionButton(COLLECTION_COPY.removeDescriptions, async anchor => {
        const accepted = await anchoredConfirmation({ document, anchor, question: COLLECTION_COPY.removeDescriptionsQuestion, actions: [
          { label: COLLECTION_COPY.removeDescriptions, value: true, danger: true }, { label: COLLECTION_COPY.cancel, value: false }
        ], datasetKey: "collectionConfirmation" });
        if (!accepted) return;
    ...
    function refreshPackage() {
      if (!activeCollection || !activeTable || !fieldsHost || !contextHost) return;
      const files = packageFiles(context);
      ...
      renderCollectionContext(contextHost, activeCollection);
      activeTable.setSchema(plainObject(schema) ? schema : undefined);
    }

    function renderCollectionContext(parent, collection) {
      parent.replaceChildren();
      const outlineEntity = context.internal.view()?.mechanisms?.find(mechanism => mechanism.id === "collections")
        ?.entities.find(entity => entity.name === collection);
      if (outlineEntity?.citations?.length) {
        const rows = locationRows(context, outlineEntity.citations, { reveal: citation => context.internal.revealLocation(citation) });
        const citations = referenceGroup(context, { label: INSPECTOR_COPY.citations,
          help: INSPECTOR_COPY.citationsHelp, rows: [rows] });
        citations.dataset.collectionCitations = "";
        parent.append(citations);
      }
      for (const offer of outlineEntity?.linkOffers ?? []) {
        if (offer.refused) {
          const sentence = element(document, "p");
          sentence.dataset.collectionLinkOffer = "refused";
          renderBackticks(sentence, WIDGET_COPY.noFieldForm(offer.refusalField));
          parent.append(sentence);
        } else {
          const action = actionButton(WIDGET_COPY.linkOffer(offer.field, offer.to),
            () => context.internal.acceptLinkOffer(outlineEntity, offer), "collectionLinkOffer");
    ...
    function render() {
      ...
      const header = element(document, "header", undefined, "opengdd-author-entity-header"); header.dataset.collectionHeader = "";
      ...
      const show = files.has(labelFile) ? actionButton(INSPECTOR_COPY.showInFile(labelFile), () => context.services.selection.select({
        kind: "file", name: "_collection.json", file: labelFile
      ...
      const actions = element(document, "div", undefined, "opengdd-author-entity-actions");
      actions.append(rename, remove, ...(show ? [show] : []), add, findLabel);
      const title = element(document, "h2", collection, "opengdd-author-identifier");
      const kind = element(document, "p", "collection", "opengdd-author-classification");
      const count = element(document, "p", undefined, "opengdd-author-form-help"); count.append(headerCount);
      header.append(title, kind, actions, count); body.append(header);
      const schema = parseCollectionJson(files.get(`collections/${collection}/_collection.json`))?.record;
      ...
      contextHost = element(document, "div"); contextHost.dataset.collectionContext = ""; body.append(contextHost);
      renderCollectionContext(contextHost, collection);
    ...
    const validationCancel = context.services.validation.subscribe(refreshPackage);
    render();
    return { destroy() { selectionCancel(); packageCancel(); validationCancel(); activeTable?.destroy?.(); body.remove(); } };
  }
};
